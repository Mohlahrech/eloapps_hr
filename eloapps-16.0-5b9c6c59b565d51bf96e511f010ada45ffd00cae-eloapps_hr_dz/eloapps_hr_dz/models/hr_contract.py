...
# Mode de paiement
class HrModePay(models.Model):
    _name = 'hr.payement.mode'
...
class HrContract(models.Model):
    ...
    def action_view_contract_wizard(self):
        ...
        created_id = self.env['hr.contract.report'].create(
        ...
        return self.env['hr.contract.report'].wizard_view_report(created_id.id)
    ...
    dispositif = fields.Many2one('hr.dispositif',string='Dispositif',)
    contrat = fields.Many2one('hr.contract.anem',string='Contrat',)
    type_contrat = fields.Many2one('hr.contract.type.anem',string='Type de Contrat',)
    ...
    payement_mode = fields.Many2one(
        ...
        default=lambda self: self.env['hr.payement.mode'].search([], limit=1))

    motif1 = fields.Many2one('hr.reason.end.contract', string='Motif de fin de contrat',)
    ...
    iep = fields.Many2one('pay.iep', string="IEP")
    ...
    def create(self, vals):
        ...
        if res.structure_type_id.name not in ['Consultant','Stagiaire']:
            ...
            allocation = self.env['hr.leave.allocation'].search([('employee_id','=',res.employee_id.id),('allocation_type','=','accrual')])
            allocation_day = 0
            if res.base_mois == 22:
                allocation_day = 1.84
...
# Dispositif ANEM
class HrDispositif(models.Model):
    _name = 'hr.dispositif'
    ...
    def name_get(self):
        res = super(HrDispositif, self).name_get()
        data1 = []
        for record in self:
...
class HrContractAnem(models.Model):
    _name = 'hr.contract.anem'
    ...
    @api.depends('name','code')
    def name_get(self):
        res = super(HrContractAnem, self).name_get()
        data2 = []
        for record in self:
            if record.name and record.code:
                display_value = record.code +":" +record.name 
...
class HContratrTypeAnem(models.Model):
    _name = 'hr.contract.type.anem'
    ...
    @api.depends('name','code')
    def name_get(self):
        res = super(HContratrTypeAnem, self).name_get()
        data3 = []
        for record in self:
            if record.name and record.code:
                display_value = record.code +":" +record.name 
